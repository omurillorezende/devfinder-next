...
async function getUser(username: string): Promise<GitHubUser | null> {
  const res = await fetch(absoluteUrl(`/api/github/user?u=${username}`), { cache: "no-store" });
  if (!res.ok) return null;
  return res.json();
}

async function getTopRepos(username: string): Promise<TopRepo[] | null> {
  const res = await fetch(absoluteUrl(`/api/github/repos?u=${username}`), { cache: "no-store" });
  if (!res.ok) return null;
  return res.json();
}
  ...
  return (
    <main className="min-h-screen grid place-items-center p-8">
      <div className="max-w-3xl w-full space-y-6">
        ...
        <UserCard data={user} />
        {repos?.length ? <TopReposChart data={repos} /> : <p className="text-sm opacity-70">Sem repositórios populares.</p>}
